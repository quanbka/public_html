 ...
 */
if (!class_exists('KendallElatedStickyNavigationWalker')) {
	class KendallElatedStickyNavigationWalker extends Walker_Nav_Menu {
		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if($depth==0){
				...
				}elseif($item->type_menu === 'wide_full_width'){
					$menu_type = " wide wide_full_width";
			...
			if($item->hide == ""){
				...
				if(empty($item->icon_pack)) {
					$item->icon_pack = $icon_pack;
				}

				$item_output .= '<span class="menu_icon_wrapper">';
				$item_output .=  kendall_elated_icon_collections()->renderIcon( $item->icon, $item->icon_pack );
				$item_output .=	'</span>';
				...
				if($args->has_children && $angle_icon != "") {
					$item_output .= '<i class="eltd-menu-arrow '.$angle_icon.'"></i>';
				}
