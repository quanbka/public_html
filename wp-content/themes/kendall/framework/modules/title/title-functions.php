...
if(!function_exists('kendall_elated_get_title')) {
    /**
     * Loads title area HTML
     */
    function kendall_elated_get_title() {
        $id = kendall_elated_get_page_id();

        extract(kendall_elated_title_area_height());
        extract(kendall_elated_title_area_background());


        //check if title area is visible on page first, then in the options
        $show_title_area = kendall_elated_get_meta_field_intersect( 'show_title_area' );

        //check for title type on page first, then in options
        $type = kendall_elated_get_meta_field_intersect( 'title_area_type' );

        //check if breadcrumbs are enabled on page first, then in options
        $enable_breadcrumbs = kendall_elated_get_meta_field_intersect( 'title_area_enable_breadcrumbs' );

        //check title size on page first, then in options
        $title_size = kendall_elated_get_meta_field_intersect( 'title_size' );
        $title_size_class = 'eltd-title-size-'.$title_size;
        ...
        $parameters = apply_filters('kendall_elated_title_area_height_params', $parameters);

        kendall_elated_get_module_template_part('templates/title', 'title', '', $parameters);
    }
}

if(!function_exists('kendall_elated_get_title_text')) {
    /**
     * Function that returns current page title text. Defines kendall_elated_title_text filter
     ...
     */
    function kendall_elated_get_title_text() {
        ...
        if (is_tag()) {
            ...
            $title = single_term_title("", false).esc_html__(' Tag', 'kendall');
        }
        ...
        elseif (is_author()) {
            ...
            $title = esc_html__('Author:', 'kendall') . " " . get_the_author();
        }
        ...
        elseif (is_singular('post')) {
            if (kendall_elated_options()->getOptionValue('blog_single_title_in_title_area') == "yes") {
        ...
        elseif (is_404()) {
            ...
            if(kendall_elated_options()->getOptionValue('404_title') != "") {
                //get it from options
                $title = kendall_elated_options()->getOptionValue('404_title');
            } else {
                //get default 404 page title
                $title = esc_html__('404 - Page not found', 'kendall');
            }
        }

        //is WooCommerce installed and is shop or single product page?
        elseif(kendall_elated_is_woocommerce_installed() && (is_shop() || is_singular('product'))) {
        ...
        elseif(kendall_elated_is_woocommerce_installed() && (is_product_category() || is_product_tag())) {
            global $wp_query;
        ...
        elseif (is_archive()) {
            $title = esc_html__('Archive','kendall');
        }
        ...
        $title = apply_filters('kendall_elated_title_text', $title);

        return $title;
    }
}

if(!function_exists('kendall_elated_title_text')) {
    /**
     * Function that echoes title text.
     *
     * @see kendall_elated_get_title_text()
     */
    function kendall_elated_title_text() {
        echo kendall_elated_get_title_text();
    }
}

if(!function_exists('kendall_elated_subtitle_text')) {
    /**
     * Function that echoes subtitle text.
     *
     */
    function kendall_elated_subtitle_text() {
        $id = kendall_elated_get_page_id();
        $subtitle 	= '';
...
if(!function_exists('kendall_elated_custom_breadcrumbs')) {
    /**
     ...
     * @see is_front_page()
     * @see is_category()
     * @see kendall_elated_is_product_category()
     * @see get_search_query()
     */
    function kendall_elated_custom_breadcrumbs() {
        global $post;

        $output = "";
        $homeLink = esc_url(home_url('/'));
        $pageid = kendall_elated_get_page_id();
        $bread_style = "";
        ...
        $delimiter = '<span class="eltd-delimiter" '.kendall_elated_get_inline_style($bread_style).'>&nbsp;/&nbsp;</span>'; // delimiter between crumbs
        $home = get_bloginfo('name'); // text for the 'Home' link
        $showCurrent = 1; // 1 - show current post/page title in breadcrumbs, 0 - don't show
        $before = '<span class="eltd-current" '.kendall_elated_get_inline_style($bread_style).'>'; // tag before the current crumb
        $after = '</span>'; // tag after the current crumb

        if (is_home() && !is_front_page()) {
            $output = '<div class="eltd-breadcrumbs"><div class="eltd-breadcrumbs-inner"><a '.kendall_elated_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a>' . $delimiter . ' <a '.kendall_elated_get_inline_style($bread_style).' href="' . $homeLink . '">'. get_the_title($pageid) .'</a></div></div>';
        ...
        elseif(is_front_page()) {
            if ($showOnHome == 1) $output = '<div class="eltd-breadcrumbs"><div class="eltd-breadcrumbs-inner"><a '.kendall_elated_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a></div></div>';
        }

        else {

            $output .= '<div class="eltd-breadcrumbs"><div class="eltd-breadcrumbs-inner"><a '.kendall_elated_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a>' . $delimiter;

            if ( is_category() || kendall_elated_is_product_category()) {
                ...
                $output .= $before . esc_html__('Search results for ', 'kendall') . '"' . get_search_query() . '"' . $after;

            } elseif ( is_day() ) {
                $output .= '<a '.kendall_elated_get_inline_style($bread_style).' href="' . get_year_link(get_the_time('Y')) . '">' . get_the_time('Y') . '</a>' . $delimiter;
                $output .= '<a '.kendall_elated_get_inline_style($bread_style).' href="' . get_month_link(get_the_time('Y'),get_the_time('m')) . '">' . get_the_time('F') . '</a>' . $delimiter;
                ...
                $output .= '<a '.kendall_elated_get_inline_style($bread_style).' href="' . get_permalink($parent) . '">' . $parent->post_title . '</a>';
                ...
                while ($parent_id) {
                    ...
                    $breadcrumbs[] = '<a '.kendall_elated_get_inline_style($bread_style).' href="' . get_permalink($page->ID) . '">' . get_the_title($page->ID) . '</a>';
                ...
                $output .= $before . esc_html__('Posts tagged ','kendall') .'"' . single_tag_title('', false) . '"' . $after;

            } elseif ( is_author() ) {
                global $authordata;
                $output .= $before . esc_html__('Articles posted by ','kendall') . $authordata->display_name . $after;
            ...
            if ( get_query_var('paged') ) {
                ...
                $output .= $before . " (" . esc_html__('Page', 'kendall') . ' ' . get_query_var('paged') . ")" . $after;
...
if(!function_exists('kendall_elated_get_title_area_height')) {

    /**
     * Function that returns title height
     **/
    function kendall_elated_get_title_area_height() {

        $id = kendall_elated_get_page_id();

        $title_type = kendall_elated_get_meta_field_intersect( 'title_area_type' );
        ...
        } elseif(kendall_elated_options()->getOptionValue('title_area_height') !== '') {
            $title_height = kendall_elated_options()->getOptionValue('title_area_height');
        }

        return apply_filters('kendall_elated_title_area_height', $title_height);
    }
}

if(!function_exists('kendall_elated_get_title_content_padding')) {
    /**
     * Function that returns title content pading
     **/

    function kendall_elated_get_title_content_padding() {
        return apply_filters('kendall_elated_title_content_padding', 0);
    }
}


if(!function_exists('kendall_elated_title_area_height')) {
    /**
     * Function that returns title height and padding to be applied in template
     **/

    function kendall_elated_title_area_height() {
        $title_height_and_padding = array();
        $title_height          = kendall_elated_get_title_area_height();
        $header_height_padding = kendall_elated_get_title_content_padding();
        $title_holder_height = '';
        ...
        $title_vertical_alignment = kendall_elated_get_meta_field_intersect( 'title_area_vertial_alignment' );
...
if(!function_exists('kendall_elated_title_area_background')) {
    /**
     * Function that returns title background style be applied in template
     *
     * @return array
     */
    function kendall_elated_title_area_background() {
        $id = kendall_elated_get_page_id();
        $show_title_img = true;
        ...
        $background_color = kendall_elated_get_meta_field_intersect( 'title_area_background_color' );

        //check if background image is set on page or in options
        $background_image = kendall_elated_get_meta_field_intersect( 'title_area_background_image' );

        //check for background image width
        $background_image_width = "";
        if($background_image !== ''){
            $background_image_width_dimensions_array = kendall_elated_get_image_dimensions($background_image);
