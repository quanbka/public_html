...
/*
  Class: KendallElatedIconCollections
...
class KendallElatedIconCollections {
	...
	private function initIconCollections() {
		$this->addIconCollection( 'font_awesome', new KendallElatedIconsFontAwesome( "Font Awesome", "fa_icon" ) );
		$this->addIconCollection( 'font_elegant', new KendallElatedIconsFontElegant( "Font Elegant", "fe_icon" ) );
		$this->addIconCollection( 'ion_icons', new KendallElatedIonIcons( "Ion Icons", "ion_icon" ) );
		$this->addIconCollection( 'linea_icons', new KendallElatedLineaIcons( 'Linea Icons', 'linea_icon' ) );
		$this->addIconCollection( 'simple_line_icons', new KendallElatedSimpleLineIcons( 'Simple Line Icons', 'simple_line_icons' ) );
		$this->addIconCollection( 'dripicons', new KendallElatedDripicons( 'Dripicons', 'dripicon' ) );
		$this->addIconCollection( 'linear_icons', new KendallElatedLinearIcons( 'Linear Icons', 'linear_icons' ) );
	}
	...
	public function getSocialVCParamsArray( $iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false, $exclude = '' ) {
		...
		$iconPackParams = array(
			'type'        => 'dropdown',
			'heading'     => esc_html__('Icon pack', 'kendall'),
			'param_name'  => $this->iconPackParamName,
			'value'       => $iconCollectionsVC,
			'save_always' => true
		);
		...
		if ( is_array( $iconCollections ) && count( $iconCollections ) ) {
			foreach ( $iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'        => 'dropdown',
					'class'       => '',
					'heading'     => esc_html__('Icon', 'kendall'),
					'param_name'  => $iconCollectionPrefix . $collection->param,
	...
	public function getSocialIconWidgetParamsArray() {
		...
		$iconPackParams[] = array(
			'type'    => 'dropdown',
			'title'   => esc_html__('Icon Pack', 'kendall'),
			'name'    => 'icon_pack',
			'options' => array(
				'font_awesome'      => esc_html__('Font Awesome', 'kendall'),
				'font_elegant'      => esc_html__('Font Elegant', 'kendall'),
				'ion_icons'         => esc_html__('Ion Icons', 'kendall'),
				'simple_line_icons' => esc_html__('Simple Line Icons', 'kendall')
			)
		);
		...
		if ( is_array( $iconCollectionsVC ) && count( $iconCollectionsVC ) ) {
			foreach ( $iconCollectionsVC as $key => $collection ) {
				$iconSetParams[] = array(
					'type'    => 'dropdown',
					'title'   => $collection->title . esc_html__(' Icon', 'kendall'),
	...
	public function getIconCollectionIcons( iKendallElatedIconCollection $collection ) {
...
if ( ! function_exists( 'kendall_elated_activate_theme_icons' ) ) {

	function kendall_elated_activate_theme_icons() {
		global $kendall_elated_IconCollections;
		$kendall_elated_IconCollections = KendallElatedIconCollections::get_instance();
	}

	add_action( 'after_setup_theme', 'kendall_elated_activate_theme_icons' );
}