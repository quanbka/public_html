...
class KendallElatedFramework {
    ...
    private function __construct() {
        $this->eltdOptions = KendallElatedOptions::get_instance();
        $this->eltdMetaBoxes = KendallElatedMetaBoxes::get_instance();
    }
	...
	public function setSkin(KendallElatedSkinAbstract $skinObject) {
		$this->skin = $skinObject;
	}
}

/**
 * Class KendallElatedSkinManager
 *
...
/**
 * Class KendallElatedSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class KendallElatedSkinAbstract {
    /**
     * @var string
     */
    protected $skinName;
    ...
    public function loadTemplatePart($template, $params = array()) {
        ...
        if($template !== '') {
            include(kendall_elated_find_template_path('framework/admin/skins/'.$this->skinName.'/templates/'.$template.'.php'));
        }
    }
    ...
    public function setShortcodeJSParams() { ?>
		<script>
			window.eltdSCIcon = '<?php echo kendall_elated_get_skin_uri().'/assets/img/admin-logo-icon.png'; ?>';
...
class KendallElatedOptions {
    ...
    public function getOptionValue($key) {
        ...
        if(array_key_exists($key, $kendall_elated_options)) {
            return $kendall_elated_options[$key];
...
/*
   Class: KendallElatedAdminPage
   A class that initializes Elated Admin Page
*/
class KendallElatedAdminPage implements iKendallElatedLayoutNode {
    ...
    function __construct($slug="", $title="", $icon = "") {
        $this->layout = array();
        $this->factory = new KendallElatedFieldFactory();
        $this->slug = $slug;
        $this->title = $title;
        $this->icon = $icon;
    }
    ...
    public function renderChild(iKendallElatedRender $child) {
        $child->render($this->factory);
    }
}

/*
   Class: KendallElatedMetaBoxes
...
/*
   Class: KendallElatedMetaBox
   A class that initializes Elated Meta Box
*/
class KendallElatedMetaBox implements iKendallElatedLayoutNode {
	...
	public function setTitle($label) {
		...
		return $kendall_elated_Framework->getSkin()->getSkinLabel().' '.$label;
 	}
}

if ( ! function_exists( 'kendall_elated_init_framework_variable' ) ) {

    function kendall_elated_init_framework_variable() {

        global $kendall_elated_Framework;

        $kendall_elated_Framework = KendallElatedFramework::get_instance();
        $eltdSkinManager = new KendallElatedSkinManager();
        $kendall_elated_Framework->setSkin($eltdSkinManager->getSkin());
    }

    add_action('kendall_elated_before_options_map', 'kendall_elated_init_framework_variable' );
}
?>