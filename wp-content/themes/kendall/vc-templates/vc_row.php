...
if( ! empty($row_type) && $row_type == 'parallax'){
	...
	if(kendall_elated_options()->getOptionValue('parallax_on_off') == 'off'){
...
if($row_type == 'row' && $css_animation != ''){
	...
	if($transition_delay !== ''){
		...
		$inner_start .= '<div '.kendall_elated_get_inline_style($animation_styles).'>';
		$inner_end .= '</div>';
	}else{
		$inner_start .= '<div>';
		$inner_end .= '</div>';
...
if($center_bck_image === 'yes'){
    $css_classes[] = 'eltd-center-bck-image';
}

if($parallax_background_image != ''){

	$parallax_image =  wp_get_attachment_image_src($parallax_background_image, 'full');
	$wrapper_style .= 'background-image:url('.$parallax_image[0].');';
	$wrapper_attributes[] = 'data-eltd-parallax-bgnd=\''.json_encode($parallax_image).'\'';
...
if($video == 'show_video'){
	...
	if($video_image) {
		$video_output .= '<div class="eltd-mobile-video-image" ' . kendall_elated_get_inline_attr($video_mobile_style, 'style') . ')"></div>';
	}
	$video_output .= '<div ' . kendall_elated_get_class_attribute($video_overlay_class) . kendall_elated_get_inline_attr($video_overlay_style, 'style') . '></div>';
	$video_output .= '<div class="eltd-video-wrap">';
	$video_output .= '<video class="eltd-video" width="1920" height="800" '. kendall_elated_get_inline_attr($video_attrs, 'poster')  .' controls="controls" preload="auto" loop autoplay muted>';
	...
	if($v_image) {
		$video_output .= '<img ' . kendall_elated_get_inline_attr($v_image, 'src') . ' width="1920" height="800" title="No video playback capabilities" alt="'.esc_html__('video thumb','kendall').'" />';
	}
	$video_output .='</object>';
	$video_output .='</video>';
	$video_output .='</div>';
