...
class ElatedInstagramApi {
    ...
    private function fetchData($limit = '', $tag = '', $transient = array('use_transient' => false)) {
        ...
        if($this->useTransients($transient) && get_transient($transient['transient_name'])) {
            ...
            if(is_wp_error($httpResponse) || (isset($httpBody['meta']['code']) && $httpBody['meta']['code'] !== 200)) {
                ...
                $returnObject->message = esc_html__('Can\'t connect with Instagram', 'eltd_instagram_feed');
        ...
        $returnObject->message = esc_html__('Success', 'eltd_instagram_feed');
    ...
    public function getAccessToken() {
        ...
        if(empty($this->code)) {
            $returnObject->status = 'error';
            $returnObject->message = esc_html__('User has\'nt connected with Instagram', 'eltd_instagram_feed');
            return $returnObject;
        } else {
            $getTokenArgs = array(
                'body' => array(
            ...
            if(is_wp_error($httpResponse) || (isset($httpResponse['response']['code']) && $httpResponse['response']['code'] !== 200)) {
                $returnObject->status = 'error';
                $returnObject->message = esc_html__('Can\t connect with Instagram API', 'eltd_instagram_feed');
            ...
            if(isset($responseBody['access_token']) && isset($responseBody['user']['id'])) {
                ...
                $returnObject->status = 'error';
                $returnObject->message = esc_html__('Can\t get Instagram access token','eltd_instagram_feed');
        ...
        //prepare return object and return it
        $returnObject->status = 'ok';
        $returnObject->message = esc_html__('Stored access token', 'eltd_instagram_feed');
